...
void sendDataToServer(String collarID, float temperature, String nombre_vaca, int pulsaciones, String macAddress) {
    ...
    // Header para indicar si el sensor de temperatura está OK
    bool tempOk = !isnan(temperature);
    http.addHeader("X-Temp-Sensor-OK", tempOk ? "true" : "false");
    ...
    if (!tempOk) {
        Serial.println("⚠️ Nota: Temperatura OMITIDA en payload (sensor desconectado)");
    }
